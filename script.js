...
function exportPatientReport() {
    const d = {
        ...
        targetWeight: document.getElementById("fTargetWeight")?.value,
        ...
        bioAge: document.getElementById("fBioAge")?.value,
        bmi: document.getElementById("fBmi")?.value,
        bmiNote: document.getElementById("fBmiNote")?.value,
        fat: document.getElementById("fFat")?.value,
        muscles: document.getElementById("fMuscles")?.value,
        legR: document.getElementById("fLegR")?.value,
        legL: document.getElementById("fLegL")?.value,
        legT: document.getElementById("fLegT")?.value,
        armR: document.getElementById("fArmR")?.value,
        armL: document.getElementById("fArmL")?.value,
        armT: document.getElementById("fArmT")?.value,
        trunk: document.getElementById("fTrunk")?.value,
        trunkT: document.getElementById("fTrunkT")?.value,
        ...
        lowerAbdomen: document.getElementById("fLowerAbdomen")?.value,
        thighR: document.getElementById("fThighR")?.value,
        thighL: document.getElementById("fThighL")?.value,
    ...
    const val = (v, unit = "") => (v !== "" && v !== null && v !== undefined && v !== "0" && v !== 0) ? `${v}${unit ? " " + unit : ""}` : "—";

    const date = new Date().toLocaleDateString("ar-SA", { year: "numeric", month: "long", day: "numeric" });

    const reportHTML = `
    <div class="rpt-page" style="direction:rtl; font-family:'Tajawal',Tahoma,sans-serif; text-align:right; line-height:1.6; background:white; padding:35px 30px 40px 48px; margin:0; width:100%; box-sizing:border-box;">

      <!-- Header -->
      <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:3px solid #E8739A; padding-bottom:12px; margin-bottom:20px;">
        <div>
          <div style="color:#8B3A9E; font-size:21px; font-weight:bold;">عيادة التغذية — Glowia Clinic</div>
          <div style="color:#D4547F; font-weight:700; font-size:14px;">الدكتورة صبا وليد الزعبي</div>
        </div>
        <div style="text-align:left; font-size:12px; color:#555;" dir="ltr">
          sebaalzoubi03@gmail.com<br/>0982720825
        </div>
      </div>

      <div style="text-align:center; font-size:18px; font-weight:800; color:#8B3A9E; margin-bottom:22px;">تقرير المؤشرات الجسدية</div>

      <!-- اسم + تاريخ -->
      <div style="background:#FFF5F9; padding:10px 16px; border-radius:8px; margin-bottom:20px; display:flex; justify-content:space-between; font-size:13.5px;">
        <div><strong>الاسم:</strong> ${val(d.name)}</div>
        <div><strong>تاريخ التقرير:</strong> ${date}</div>
      </div>

      <!-- المؤشرات الجسدية - تخطيط مطابق للصورة -->
      <div style="margin-bottom:25px;">
        <div style="font-size:14px; font-weight:700; color:#8B3A9E; margin-bottom:10px;">المؤشرات الجسدية</div>
        
        <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:10px;">
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;">العمر البيولوجي</div>
            <div style="font-size:20px; font-weight:800; color:#8B3A9E;">${val(d.bioAge)}</div>
          </div>
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;">العمر</div>
            <div style="font-size:20px; font-weight:800; color:#8B3A9E;">${val(d.age)}</div>
          </div>
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;">الطول (CM)</div>
            <div style="font-size:20px; font-weight:800; color:#8B3A9E;">${val(d.height)}</div>
          </div>
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;">الوزن (KG)</div>
            <div style="font-size:20px; font-weight:800; color:#8B3A9E;">${val(d.weight)}</div>
          </div>
        </div>

        <div style="display:grid; grid-template-columns: 1fr 4fr ; gap:10px; margin-top:12px;">
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;"> BMI</div>
            <div style="font-size:15px; font-weight:600; color:#555;">${val(d.bmi)}</div>
          </div>
          <div style="background:#f9f0ff; border:1px solid #d4a8e0; border-radius:10px; padding:10px; text-align:center;">
            <div style="font-size:11px; color:#666;"> BMI</div>
            <div style="font-size:20px; font-weight:800; color:#d4547f;">${val(d.bmiNote)}</div>
          </div>
        </div>

        <!-- الوزن المستهدف + هدف الدهون + هدف العضلات -->
        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px; margin-top:12px;">
          <div style="background:#FFF5F9; border:2px solid #E8739A; border-radius:10px; padding:12px; text-align:center;">
            <div style="font-size:11px; color:#666;">الوزن المراد الوصول له (KG)</div>
            <div style="font-size:22px; font-weight:900; color:#8B3A9E;">${val(d.targetWeight)}</div>
          </div>
          <div style="background:#FFF5F9; border:2px solid #E8739A; border-radius:10px; padding:12px; text-align:center;">
            <div style="font-size:11px; color:#666;">هدف الدهون (KG)</div>
            <div style="font-size:22px; font-weight:900; color:#8B3A9E;">${val(d.fat)}</div>
          </div>
          <div style="background:#FFF5F9; border:2px solid #E8739A; border-radius:10px; padding:12px; text-align:center;">
            <div style="font-size:11px; color:#666;">هدف العضلات (KG)</div>
            <div style="font-size:22px; font-weight:900; color:#8B3A9E;">${val(d.muscles)}</div>
          </div>
        </div>
      </div>
      ...
      <div style="margin:30px 0;">
        <div style="font-size:14px; font-weight:700; color:#8B3A9E; margin-bottom:10px;">تفصيل العضلات</div>
        <table style="width:100%; border-collapse:collapse; border:2px solid #d4a8e0; border-radius:10px; overflow:hidden;">
          <thead>
            <tr style="background:#E8739A; color:white;">
              <th style="padding:12px;">العضلات</th>
              <th style="padding:12px;">اليمين</th>
              <th style="padding:12px;">اليسار</th>
              <th style="padding:12px;">الهدف</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td style="padding:12px; font-weight:600; border-bottom:1px solid #eee;">الذراعين</td>
              <td style="padding:12px; text-align:center; border-bottom:1px solid #eee;">${val(d.armR, "كجم")}</td>
              <td style="padding:12px; text-align:center; border-bottom:1px solid #eee;">${val(d.armL, "كجم")}</td>
              <td style="padding:12px; text-align:center; color:#8B3A9E; font-weight:700;">${val(d.armT, "كجم")}</td>
            </tr>
            <tr>
              <td style="padding:12px; font-weight:600; border-bottom:1px solid #eee;">الساقين</td>
              <td style="padding:12px; text-align:center; border-bottom:1px solid #eee;">${val(d.legR, "كجم")}</td>
              <td style="padding:12px; text-align:center; border-bottom:1px solid #eee;">${val(d.legL, "كجم")}</td>
              <td style="padding:12px; text-align:center; color:#8B3A9E; font-weight:700;">${val(d.legT, "كجم")}</td>
            </tr>
            <tr>
              <td style="padding:12px; font-weight:600;">الجذع</td>
              <td style="padding:12px; text-align:center;">${val(d.trunk, "كجم")}</td>
              <td style="padding:12px; text-align:center;">—</td>
              <td style="padding:12px; text-align:center; color:#8B3A9E; font-weight:700;">${val(d.trunkT, "كجم")}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div style="page-break-before:always;">
        <div style="font-size:15px; font-weight:700; color:#8B3A9E; margin:25px 0 12px;">ملاحظات الدكتورة</div>
        <div style="border:1.5px solid #E8739A; border-radius:12px; padding:18px; min-height:160px; background:#fff9fb; white-space:pre-wrap;">
          ${d.notes || ""}
        </div>

        <div style="margin-top:35px;">
          <div style="font-size:15px; font-weight:700; color:#8B3A9E; margin-bottom:12px;">ملاحظات الكوتش</div>
          <div style="border:2px dashed #8B3A9E; border-radius:12px; padding:25px; min-height:240px; background:#f8f8f8;">
            <!-- مساحة فارغة للكتابة باليد -->
          </div>
        </div>
      </div>
